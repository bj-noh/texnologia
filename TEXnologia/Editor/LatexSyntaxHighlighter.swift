...
final class LatexSyntaxHighlighter {
    ...
    func apply(to textStorage: NSTextStorage?, text: String, settings: AppSettings) {
    ...
    func isSpellCheckExcluded(_ range: NSRange, in text: String) -> Bool {
        spellCheckExcludedRanges(in: text).contains { excludedRange in
            NSIntersectionRange(range, excludedRange).length > 0
        }
    }

    func spellCheckExcludedRanges(in text: String) -> [NSRange] {
        let nsText = text as NSString
        let fullRange = NSRange(location: 0, length: nsText.length)
